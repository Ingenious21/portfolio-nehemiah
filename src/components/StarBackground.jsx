...
export const StarBackground = () => {
  ...
  // Programming icons data
  const programmingIconsData = useMemo(() => [
    { 
      name: "HTML5", 
      color: "#E34F26",
    ...
    { 
      name: "CSS3", 
      color: "#1572B6",
      svg: `<svg viewBox="0 0 24 24" fill="#1572B6"><path d="M1.5 0h21l-1.91 21.563L11.977 24l-8.564-2.438L1.5 0zm4.133 4.568l2.83 31.704 8.536-2.363 8.566-2.364L8.63 4.568z"/></svg>`
    },
    { 
      name: "JavaScript", 
      color: "#F7DF1E",
      svg: `<svg viewBox="0 0 24 24" fill="#F7DF1E"><path d="M0 0h24v24H0V0zm22.034 18.276c-.175-1.095-.888-2.015-3.003-2.873-.736-.345-1.554-.585-1.797-1.14-.091-.33-.105-.51-.046-.705.15-.646.915-.84 1.515-.66.39.12.75.42.976.9 1.034-.676 1.034-.676 1.755-1.125-.27-.42-.404-.601-.586-.78-.63-.705-1.469-1.065-2.834-1.034l-.705.089c-.676.165-1.32.525-1.71 1.005-1.14 1.291-.811 3.541.569 4.471 1.365 1.02 3.361 1.244 3.616 2.205.24 1.17-.87 1.545-1.966 1.41-.811-.18-1.26-.586-1.755-1.336l-1.83 1.051c.21.48.45.689.81 1.109 1.74 1.756 6.09 1.666 6.871-1.004.029-.09.24-.705.074-1.65l.046.067zm-8.983-7.245h-2.248c0 1.938-.009 3.864-.009 5.805 0 1.232.063 2.363-.138 2.711-.33.689-1.18.601-1.566.48-.396-.196-.597-.466-.83-.855-.063-.105-.11-.196-.127-.196l-1.825 1.125c.305.63.75 1.172 1.324 1.517.855.51 2.004.675 3.207.405.783-.226 1.458-.691 1.811-1.411.51-.98.402-2.178.398-3.502.012-2.022 0-4.045 0-6.07z"/></svg>`
    },
    { 
      name: "React", 
      color: "#61DAFB",
  ...
  // Generate meteors for dark mode
  ...
  // Generate programming icons for light mode with fluid sizing
  const generateProgrammingIcons = useCallback(() => {
    const numberOfIcons = window.innerWidth < 640 ? 6 : window.innerWidth < 1024 ? 8 : 10;
    const newIcons = [];

    for (let i = 0; i < numberOfIcons; i++) {
      const iconData = programmingIconsData[i % programmingIconsData.length];
      newIcons.push({
        id: i,
        ...iconData,
        x: Math.random() * 80 + 10,
        y: Math.random() * 80 + 10,
        // Size will be controlled by CSS clamp(), but we keep this for reference
        size: 'clamp(28px, 4vw, 60px)',
  ...
  }, [generateStars, generateMeteors, generateProgrammingIcons]);
      ...
      {isDarkMode && (
        <>
          {stars.map((star) => (
            <div
              key={star.id}
              className="star animatePulseSubtle"
              style={{
                width: star.size + "px",
                height: star.size + "px",
                left: star.x + "%",
          ...
          {meteors.map((meteor) => (
            <div
              key={meteor.id}
              className="meteor animateMeteor"
              style={{
      ...
      {!isDarkMode && (
        <>
          {programmingIcons.map((icon) => (
            <div
              key={icon.id}
              className="programmingIcon animateFloatProgramming"
              data-color={icon.color}
              style={{
                left: icon.x + "%",
                top: icon.y + "%",
                // Size is now controlled by CSS clamp(), no inline width/height needed
